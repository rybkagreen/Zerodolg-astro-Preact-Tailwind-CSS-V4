...
describe('TrustBadges Component', () => {
  ...
  it('should handle trust badge verification correctly', () => {
    ...
    expect(sortedByVerification[0].id).toBe('license'); // Verified first (Лицензия)
    expect(sortedByVerification[1].id).toBe('experience'); // Verified second (Опыт)
    expect(sortedByVerification[2].id).toBe('support'); // Verified third (Поддержка)
    expect(sortedByVerification[3].id).toBe('guarantee'); // Unverified last (Гарантия)
  ...
  it('should include proper accessibility attributes', () => {
    ...
    trustBadgesSubtitle.textContent =
      'Более 1450 клиентов уже выбрали нас для решения своих долговых проблем';
  ...
  it('should handle performance considerations correctly', () => {
    ...
    const titleSorted = sortLargeDataset(largeTrustBadgeDataset, (a, b) =>
