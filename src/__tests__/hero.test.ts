...
describe('Hero Component', () => {
  ...
  it('should have correct hero content structure', () => {
    ...
    // Verify content text
    expect(hero.querySelector('.hero__title')?.textContent).toBe(
      'Освободитесь от долгов раз и навсегда'
    );
    expect(hero.querySelector('.hero__subtitle')?.textContent).toBe(
      'Профессиональная помощь в процедуре банкротства физических лиц'
    );
    expect(hero.querySelector('.hero__description')?.textContent).toBe(
      '98% успешных дел • От 5 900 ₽/мес • Без скрытых платежей'
    );
  });
  ...
  it('should handle countdown timer correctly', () => {
    ...
    const countdownItemsElements = document.querySelectorAll(
      '[class*="hero__countdown-"]:not([class$="-value"]):not([class$="-label"])'
    );
    ...
    const updateCountdown = (days: string, hours: string, minutes: string, seconds: string) => {
      ...
      const minutesElement = document.querySelector(
        '.hero__countdown-minutes .hero__countdown-value'
      );
      const secondsElement = document.querySelector(
        '.hero__countdown-seconds .hero__countdown-value'
      );
