...
describe('Performance Tests', () => {
  ...
  const mockPerformance = () => {
    ...
    global.performance = {
      ...
      getEntriesByName: vi.fn().mockImplementation((name: string) => {
        return [...marks.filter(m => m.name === name), ...measures.filter(m => m.name === name)];
      }),
      clearMarks: vi.fn(),
      clearMeasures: vi.fn(),
  ...
  it('should measure rendering performance correctly', () => {
    ...
    expect(performance.measure).toHaveBeenCalledWith('timeline-render', 'timeline-start', 'timeline-end');
  });
    ...
    const bundleAnalyzer = {
      ...
      getTotalSize: function(gzipped: boolean = false) {
        return Object.values(this.bundles)
          .reduce((sum, bundle) => sum + (gzipped ? bundle.gzip : bundle.size), 0);
      },
  ...
  it('should measure network performance correctly', () => {
    const networkMonitor = {
      ...
      getAverageResponseTime: function() {
        ...
        const totalTime = this.requests.reduce((sum, req) => sum + (req.endTime - req.startTime), 0);
  ...
  it('should measure animation performance correctly', () => {
    const animationProfiler = {
      animations: new Map<string, {frameCount: number, totalTime: number, frameTimes: number[]}>(),
  ...
  it('should measure cache performance correctly', () => {
    const cacheProfiler = {
      ...
      hitTimes: [] as number[],
      missTimes: [] as number[],
      recordHit: function(time: number = 1) { // Cache hits are typically fast
        this.cacheHits++;
        this.hitTimes.push(time);
      },
      recordMiss: function(time: number = 10) { // Cache misses are typically slower
        this.cacheMisses++;
  ...
  it('should measure resource loading performance correctly', () => {
    const resourceLoader = {
      resources: new Map<string, {loaded: boolean, loadTime: number, size: number, type: string}>(),
