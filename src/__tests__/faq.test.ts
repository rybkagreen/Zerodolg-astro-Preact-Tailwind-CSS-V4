...
describe('Faq Component', () => {
  ...
  it('should handle FAQ filtering and sorting correctly', () => {
    const faqItems = [
      { id: '1', question: 'Вопрос A', category: 'debts', helpful: 24, date: '2024-03-15', verified: true },
      { id: '2', question: 'Вопрос B', category: 'timeline', helpful: 18, date: '2024-03-10', verified: true },
      { id: '3', question: 'Вопрос C', category: 'property', helpful: 31, date: '2024-03-05', verified: true },
      { id: '4', question: 'Вопрос D', category: 'documents', helpful: 12, date: '2024-03-20', verified: false },
      { id: '5', question: 'Вопрос E', category: 'costs', helpful: 8, date: '2024-03-01', verified: true }
    ];
  ...
  it('should handle performance considerations correctly', () => {
    ...
    for (let i = 0; i < 1000; i++) {
      largeFAQDataset.push({
        ...
        category: i % 5 === 0 ? 'debts' : i % 5 === 1 ? 'timeline' : i % 5 === 2 ? 'property' : i % 5 === 3 ? 'documents' : 'costs',
    ...
    const renderMeasure = performanceMock.measure('faq-render', 'faq-render-start', 'faq-render-end');
    ...
    expect(sortedItems[sortedItems.length - 2].helpful).toBeGreaterThanOrEqual(sortedItems[sortedItems.length - 1].helpful);
    
    // Test pagination performance
    performanceMock.mark('paginate-start');
    
    const paginateLargeDataset = (items: typeof largeFAQDataset, page: number, pageSize: number) => {
      const startIndex = (page - 1) * pageSize;
      const endIndex = startIndex + pageSize;
      return items.slice(startIndex, endIndex);
    };
    ...
    const paginateMeasure = performanceMock.measure('faq-paginate', 'paginate-start', 'paginate-end');
