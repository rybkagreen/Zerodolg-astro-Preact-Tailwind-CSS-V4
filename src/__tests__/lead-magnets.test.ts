...
describe('LeadMagnets Component', () => {
  ...
  it('should handle lead magnet data correctly', () => {
    ...
    expect(checklistMagnet.description).toBe('Бесплатный чек-лист с пошаговым планом процедуры банкротства');
    ...
    expect(guideMagnet.description).toBe('Подробное руководство с юридическими аспектами и практическими советами');
    ...
    expect(calculatorMagnet.description).toBe('Узнайте, сколько вы можете сэкономить с помощью процедуры банкротства');
    ...
    expect(templateMagnet.description).toBe('Готовые шаблоны заявлений и документов для процедуры банкротства');
  ...
  it('should handle lead magnet filtering and categorization correctly', () => {
    ...
    const filterByCategory = (magnets: typeof leadMagnets, category: string) => {
      return category === 'all' ? magnets : magnets.filter(magnet => magnet.category === category);
    };
  ...
  it('should include proper accessibility attributes', () => {
    ...
    const leadMagnets = [
      { id: 'checklist', title: 'Чек-лист: 10 шагов к банкротству', description: 'Пошаговый план процедуры банкротства' },
      { id: 'guide', title: 'Полное руководство по банкротству физлиц', description: 'Подробное руководство с юридическими аспектами' },
      { id: 'calculator', title: 'Калькулятор экономии при банкротстве', description: 'Узнайте, сколько вы можете сэкономить' },
      { id: 'template', title: 'Шаблоны документов для банкротства', description: 'Готовые шаблоны заявлений и документов' }
    ];
    ...
    expect(leadMagnetsSection.getAttribute('aria-label')).toBe('Бесплатные материалы по банкротству');
    expect(leadMagnetsSection.id).toBe('lead-magnets');
    
    expect(leadMagnetsHeader.querySelector('h2')?.id).toBe('lead-magnets-title');
    expect(leadMagnetsHeader.querySelector('p')?.getAttribute('aria-describedby')).toBe('lead-magnets-title');
  ...
  it('should handle performance considerations correctly', () => {
    ...
    for (let i = 0; i < 1000; i++) {
      largeLeadMagnetDataset.push({
        ...
        category: i % 4 === 0 ? 'checklists' : i % 4 === 1 ? 'guides' : i % 4 === 2 ? 'calculators' : 'templates'
      });
    }
    
    performanceMock.mark('lead-magnets-render-end');
    const renderMeasure = performanceMock.measure('lead-magnets-render', 'lead-magnets-render-start', 'lead-magnets-render-end');
    ...
    const filterMeasure = performanceMock.measure('lead-magnets-filter', 'filter-start', 'filter-end');
    ...
    const searchMeasure = performanceMock.measure('lead-magnets-search', 'search-start', 'search-end');
    ...
    const paginateLargeDataset = (items: typeof largeLeadMagnetDataset, page: number, pageSize: number) => {
    ...
    const paginateMeasure = performanceMock.measure('lead-magnets-paginate', 'paginate-start', 'paginate-end');
