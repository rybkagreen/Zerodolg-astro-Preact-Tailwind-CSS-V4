...
describe('SpecialOffers Component', () => {
  ...
  it('should handle offer filtering and sorting correctly', () => {
    ...
    const multiTagFilter = (offers: typeof specialOffers, tags: string[]) => {
      return offers.filter(offer => 
        tags.every(tag => offer.tags.includes(tag))
      );
    };
  ...
  it('should calculate offer prices correctly', () => {
    ...
    const debtReliefOriginalTotal = calculateOriginalTotal(debtReliefOriginalMonthly, debtReliefOffer.duration);
    const debtReliefActualDiscount = calculateActualDiscount(debtReliefOriginalTotal, debtReliefTotal);
    ...
    const propertyOriginalTotal = calculateOriginalTotal(propertyOriginalMonthly, propertyOffer.duration);
    ...
    const businessOriginalTotal = calculateOriginalTotal(businessOriginalMonthly, businessOffer.duration);
  ...
  it('should handle performance considerations correctly', () => {
    ...
    const renderMeasure = performanceMock.measure('offers-render', 'offers-render-start', 'offers-render-end');
    ...
    const filterLargeDataset = (items: typeof largeOfferDataset, filterFn: (item: any) => boolean) => {
    ...
    const tagFiltered = filterLargeDataset(largeOfferDataset, item => item.tags.includes('Тег 1'));
    ...
    const sortLargeDataset = (items: typeof largeOfferDataset, sortFn: (a: any, b: any) => number) => {
    ...
    const paginateLargeDataset = (items: typeof largeOfferDataset, page: number, pageSize: number) => {
    ...
    const paginateMeasure = performanceMock.measure('offers-paginate', 'paginate-start', 'paginate-end');
