...
describe('Process Component', () => {
  ...
  it('should visualize process timeline correctly', () => {
    ...
    const updateTimelineStep = (timeline: ReturnType<typeof createTimelineVisualization>, stepId: string, updates: Partial<typeof timeline[0]>) => {
  ...
  it('should handle step details correctly', () => {
    ...
    const getStepDetail = (stepId: keyof typeof processStepDetails, detailType: 'title' | 'description' | 'duration' | 'difficulty' | 'icon') => {
    ...
    expect(getStepDetail('analysis', 'description')).toBe('Изучаем вашу финансовую ситуацию и определяем перспективы банкротства');
    ...
    expect(getStepDetail('documentation', 'description')).toBe('Собираем и оформляем все необходимые документы для подачи заявления');
  ...
  it('should include proper accessibility attributes', () => {
    ...
    expect(processHeader.querySelector('p')?.getAttribute('aria-describedby')).toBe('process-title');
  ...
  it('should handle performance considerations correctly', () => {
    ...
    const renderMeasure = performanceMock.measure('process-render', 'process-render-start', 'process-render-end');
    ...
    const navigationMeasure = performanceMock.measure('process-navigation', 'navigation-start', 'navigation-end');
