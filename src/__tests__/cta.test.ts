...
describe('CTA Component', () => {
  ...
  it('should have correct CTA section structure', () => {
    ...
    // Verify content
    expect(ctaSection.querySelector('.cta__title')?.textContent).toBe('Готовы начать процедуру банкротства?');
    expect(ctaSection.querySelector('.cta__text')?.textContent).toBe('Оставьте заявку и мы поможем на каждом этапе');
    expect(ctaSection.querySelector('.cta__button')?.textContent).toBe('Получить консультацию');
    expect(ctaSection.querySelector('.cta__button')?.getAttribute('data-modal')).toBe('consultation');
  ...
  it('should handle CTA animations correctly', () => {
    ...
    expect(ctaSection.querySelector('.cta__title')?.classList.contains('animate-fade-in')).toBe(true);
    expect(ctaSection.querySelector('.cta__text')?.classList.contains('animate-fade-in')).toBe(true);
    expect(ctaSection.querySelector('.cta__button')?.classList.contains('animate-bounce-in')).toBe(true);
    ...
    expect(ctaSection.querySelector('.cta__title')?.classList.contains('animation-complete')).toBe(true);
    expect(ctaSection.querySelector('.cta__text')?.classList.contains('animated')).toBe(true);
    expect(ctaSection.querySelector('.cta__text')?.classList.contains('animation-complete')).toBe(true);
    expect(ctaSection.querySelector('.cta__button')?.classList.contains('animated')).toBe(true);
    expect(ctaSection.querySelector('.cta__button')?.classList.contains('animation-complete')).toBe(true);
    ...
    expect(ctaSection.querySelector('.cta__title')?.classList.contains('animation-complete')).toBe(false);
    expect(ctaSection.querySelector('.cta__text')?.classList.contains('animated')).toBe(false);
    expect(ctaSection.querySelector('.cta__text')?.classList.contains('animation-complete')).toBe(false);
    expect(ctaSection.querySelector('.cta__button')?.classList.contains('animated')).toBe(false);
    expect(ctaSection.querySelector('.cta__button')?.classList.contains('animation-complete')).toBe(false);
  });
  ...
  it('should include proper accessibility attributes', () => {
    ...
    ctaSection.setAttribute('aria-label', 'Призыв к действию - получить консультацию по банкротству');
    ctaSection.id = 'cta';
    ...
    expect(ctaSection.getAttribute('aria-label')).toBe('Призыв к действию - получить консультацию по банкротству');
    ...
    expect(ctaButton.className).toBe('cta__button');
    expect(ctaButton.getAttribute('data-modal')).toBe('consultation');
    expect(ctaButton.getAttribute('aria-label')).toBe('Получить бесплатную консультацию по банкротству');
  ...
  it('should handle performance considerations correctly', () => {
    ...
    const renderMeasure = performanceMock.measure('cta-render', 'cta-render-start', 'cta-render-end');
    ...
    const interactionMeasure = performanceMock.measure('cta-interaction', 'cta-interaction-start', 'cta-interaction-end');
    ...
    const cleanupMeasure = performanceMock.measure('cta-cleanup', 'cta-cleanup-start', 'cta-cleanup-end');
  ...
  it('should handle CTA form submission correctly', () => {
    ...
    const validateForm = (form: HTMLFormElement) => {
      ...
      } else if (!/^(\+7|8)[\s\-]?\(?[0-9]{3}\)?[\s\-]?[0-9]{3}[\s\-]?[0-9]{2}[\s\-]?[0-9]{2}$/.test(phoneInput.value)) {
  ...
  it('should handle analytics tracking correctly', () => {
    ...
    const trackEngagement = (engagementType: string, duration: number, analytics = analyticsMock) => {
