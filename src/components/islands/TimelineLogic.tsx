...
const TimelineLogic = () => {
  useEffect(() => {
    ...
    const progressDots = Array.from(section.querySelectorAll('.timeline__progress-dot')) as HTMLElement[];
    ...
    const trackEvent = (eventName: string, parameters: Record<string, any> = {}) => {
      ...
      // Yandex Metrika
      if (window.ym) {
        window.ym(window.yaMetrikaId || 98741026, 'reachGoal', eventName, parameters);
      } else if (window.yaCounter98741026) {
        window.yaCounter98741026.reachGoal(eventName, parameters);
      }
    };
