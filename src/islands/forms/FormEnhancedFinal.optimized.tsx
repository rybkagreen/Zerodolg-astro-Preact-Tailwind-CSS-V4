...
class FormErrorBoundary extends Component<{ children: VNode; className?: string }, { hasError: boolean; error?: Error }> {
...
const SuccessCheckmark = (): JSX.Element => (
  <div class="success-checkmark">
    ...
    <style jsx>{`
      ...
      .success-checkmark {
        animation: fill .4s ease-in-out .4s both, scale .3s ease-in-out .9s both;
      }
    `}</style>
  </div>
);
...
export default function FormEnhancedFinal({
  ...
  const [fieldValidation, setFieldValidation] = useState<Record<string, 'valid' | 'invalid' | 'neutral'>>({});
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        ...
        if (firstErrorField && formRef.current) {
          const input = formRef.current.querySelector(`[name="${firstErrorField.name}"]`) as HTMLElement;
          input?.focus();
        }
      }
      return false;
        ...
        >
          {config.fields.map((field) => (
            <div key={field.name} class="form-field">
              ...
              >
                {field.label}
                {field.required && <span class="text-error-500 ml-1" aria-label="обязательное поле">*</span>}
              </label>
              
              <div class="relative">
                {field.type === 'text' && (field.name.includes('comment') || field.name.includes('message') || field.name.includes('description')) ? (
                  <textarea
                ...
                {enableProgressiveEnhancement && touchedFields.has(field.name) && fieldValidation[field.name] && fieldValidation[field.name] === 'valid' && (
