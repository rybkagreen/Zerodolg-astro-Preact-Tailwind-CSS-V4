...
> ⚠️ Если исполнитель — Qwen Code, hook/Fable-специфику ниже адаптировать под свой runtime.
...
- В `master` не попадают: `.env*` (кроме `.env.example`), `tmp/`, `docs.archive/`, `docs/_planner/`,
  память агента (`.claude/settings.local.json` и подобное локальное состояние — но НЕ `.qwen/`,
  который в этом репозитории является трекаемой shared-конфигурацией основного AI-инструмента
  команды, см. `CLAUDE.md` → «AI Integration Guidelines»; расхождение с общей методичкой
  зафиксировано явно, не вычищено молча), `node_modules/`, артефакты сборки. Публичный репо — строго.
