...
> (планировщик) и агент-исполнитель. Александр читает для контроля. Обновлён:
> 14.07.2026 (housekeeping-промт PROMPT_02 — размещение и факт-проверка
> методички) Прод: **zerodolg.ru** — live (Astro + Islands, с октября 2025).
...
- **Minor (1.X):** дополнения, уточнения, новые записи (в т.ч. факт-проверка
  технических утверждений против репозитория, как в этом обновлении).
...
закрытие блока). Технические утверждения (версии, наличие тест-раннера,
CI-триггеры) сверять с фактическим репозиторием, не только между собой —
housekeeping-промт 14.07.2026 нашёл несколько случаев, где методичка и код
