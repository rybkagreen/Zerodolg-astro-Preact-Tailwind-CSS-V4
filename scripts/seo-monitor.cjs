...
function checkMetaTags(htmlContent) {
  ...
  const hasDescription = /<meta[^>]*name=["']description["'][^>]*content=["'][^"']*["']/.test(htmlContent);
  const hasKeywords = /<meta[^>]*name=["']keywords["'][^>]*content=["'][^"']*["']/.test(htmlContent);
...
function checkStructuredData(htmlContent) {
  const hasStructuredData = /<script[^>]*type=["']application\/ld\+json["'][^>]*>/.test(htmlContent);
...
function generateReport(results) {
  ...
  console.log(`- Files with H1 tags: ${filesWithH1}/${totalFiles} (${Math.round((filesWithH1/totalFiles)*100)}%)`);
  console.log(`- Files with meta description: ${filesWithDescription}/${totalFiles} (${Math.round((filesWithDescription/totalFiles)*100)}%)`);
  console.log(`- Files with canonical tags: ${filesWithCanonical}/${totalFiles} (${Math.round((filesWithCanonical/totalFiles)*100)}%)`);
  console.log(`- Files with structured data: ${filesWithStructuredData}/${totalFiles} (${Math.round((filesWithStructuredData/totalFiles)*100)}%)`);
  console.log(`- Files with Yandex.Metrika: ${filesWithYandexMetrika}/${totalFiles} (${Math.round((filesWithYandexMetrika/totalFiles)*100)}%)`);
  console.log(`- Files with Google Analytics: ${filesWithGoogleAnalytics}/${totalFiles} (${Math.round((filesWithGoogleAnalytics/totalFiles)*100)}%)`);
  console.log(`- Files with Bitrix24 integration: ${filesWithBitrix24}/${totalFiles} (${Math.round((filesWithBitrix24/totalFiles)*100)}%)\n`);
  
  // Detailed analysis
  console.log('Detailed Analysis:');
  results.forEach(result => {
    console.log(`\nFile: ${result.filePath.replace(path.join(__dirname, '..', 'dist'), '')}`);
    console.log(`  Headings: H1(${result.headings.h1}) H2(${result.headings.h2}) H3(${result.headings.h3}) H4(${result.headings.h4}) H5(${result.headings.h5}) H6(${result.headings.h6})`);
    console.log(`  Meta tags: Title(${result.metaTags.hasTitle ? '✓' : '✗'}) Description(${result.metaTags.hasDescription ? '✓' : '✗'}) Keywords(${result.metaTags.hasKeywords ? '✓' : '✗'}) Canonical(${result.metaTags.hasCanonical ? '✓' : '✗'})`);
    console.log(`  Structured data: ${result.structuredData.hasStructuredData ? '✓' : '✗'}`);
    console.log(`  Analytics: Yandex.Metrika(${result.analytics.hasYandexMetrika ? '✓' : '✗'}) Google Analytics(${result.analytics.hasGoogleAnalytics ? '✓' : '✗'}) Bitrix24(${result.analytics.hasBitrix24 ? '✓' : '✗'})`);
  });
}
