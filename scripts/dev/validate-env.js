...
const optionalEnvVars = {
  ...
  PUBLIC_ASTRO_TOOLBAR: {
    validator: (val) => val === 'true' || val === 'false',
    description: 'Astro Dev Toolbar flag',
  },
  NODE_ENV: {
  ...
  if (process.env[envVar]) {
    ...
    if (!validator(process.env[envVar])) {
      validationErrors.push(
        `Invalid ${optionalEnvVars[envVar].description} format: ${process.env[envVar]}`
      );
    }
  }
});
...
requiredEnvVars.forEach((envVar) => {
  ...
  const maskedValue =
    envVar.includes('WEBHOOK') || envVar.includes('KEY')
...
if (process.env.NODE_ENV !== 'production') {
  console.warn(
    '⚠️  NODE_ENV is not set to "production". Consider setting it for production builds.'
  );
}
