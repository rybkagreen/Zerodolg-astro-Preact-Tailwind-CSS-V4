#!/usr/bin/env python3
"""
Updated Nginx Log Monitor with Ollama Integration
...
class NginxLogHandler(FileSystemEventHandler):
    ...
    def _analyze_error_with_ollama(self, error_line):
        """Send error to ollama for analysis"""
        try:
            OLLAMA_API_URL = os.getenv('OLLAMA_API_URL', 'http://localhost:11434/api/generate')
            prompt = f"""
Analyze this nginx error log entry: '{error_line}'
Identify:
1. The type of error
2. Potential causes
3. Possible solutions
4. Priority level (low, medium, high, critical)
Provide a structured response that can help an automated system understand and potentially fix the issue.
"""
            
            payload = {
                "model": os.getenv('OLLAMA_ANALYSIS_MODEL', 'llama2'),  # Configurable model for analysis
                "prompt": prompt,
                "stream": False,
                "options": {
            ...
            logger.error("Cannot connect to ollama API. Please ensure ollama is running.")
            return "Ollama API connection failed: Cannot connect to ollama service"
        except requests.exceptions.Timeout:
            logger.error("Timeout while calling ollama API.")
            return "Ollama API timeout: Request took too long"
        except requests.exceptions.RequestException as e:
            logger.error(f"Request error when calling ollama API: {e}")
            return f"Ollama API request error: {str(e)}"
        except Exception as e:
            logger.error(f"Unexpected error calling ollama API: {e}")
            return f"Ollama analysis failed with unexpected error: {str(e)}"
    ...
    def _send_report_via_ssh(self, report_path):
        ...
        try:
            ...
            # Create a more specific prompt for the coder model based on the error details
            prompt = (
                f"Fix the error described in this report: {remote_path}. "
                "Analyze the error details and implement appropriate fixes in the "
                "/root/develop/zerodolg.ru/zerodolg-astro project. "
                "Focus on the specific error described in the report, "
                "apply necessary code changes, and ensure the fix is appropriate "
                "for the type of error detected. After making fixes, run the "
                "deployment script."
            )
            
            command = f"cd /root/develop/zerodolg.ru/zerodolg-astro && qwen --model '{config['CODER_MODEL_NAME']}' --prompt '{prompt}' && /root/develop/zerodolg.ru/zerodolg-astro/deploy-ssr.sh"
